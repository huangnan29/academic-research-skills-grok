...
def is_codex_distribution() -> bool:
    """兼容使用 WORKFLOW.md 入口的 Codex 与 Grok Build 发行版。"""
    return codex_manifest().get("generated_for") in {"codex", "grok-build"}


def codex_excluded_patterns() -> tuple[str, ...]:
    manifest = codex_manifest()
    if manifest.get("generated_for") not in {"codex", "grok-build"}:
        return ()

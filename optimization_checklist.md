# ARS-Grok Build 功能优化清单

## v0.3.1 验证优先

- [x] Grok 1.0.13 init 工具表：已实测；直接Agent额外暴露MCP，严格隔离未通过
- [ ] 三Agent真实产物交接：不以启动成功证明流水线完成
- [x] 新增自然语言实际 Skill 加载轨迹验收器，不向模型提供期望路由答案；完整结果见验收报告
- [ ] Hook 工具名、工作目录和路径字段在真实事件中核验
- [x] 旧版标记测试降级为输出格式冒烟，不再作为安全认证
- [ ] 当前证据满足后才发布；插件、压缩备份和发布自动化延后

已完成的本地修复：Hook正确区分workspaceRoot/cwd、保护Grok执行绑定文件、适配终端内部别名；备份清理仅处理有所有者标记的目录；损坏轨迹和空工具回执不得误报成功。125项单元测试及核心包隔离安装通过，不能据此推定真实Grok端到端通过。

# ARS-Grok Build v0.3.1 行为证据升级计划

## 2026-09-07 实施增量

根据本轮真实测试修正固定过程披露、审稿误判、输出越界与调度归属。修正集中于Grok适配层；ars正文保持可追溯，报告角色只在固定披露节与上游明确偏离。验证采用现有根测试与隔离安装后的定向Grok任务。全局安装及稳定发布另行验收，不用提示词修正宣称平台MCP隔离已解决。

## v0.3.1 执行范围

先验证 Grok 1.0.13 的实际工具暴露、自然语言路由和三阶段文件交接，再根据证据做最小修复。使用合成夹具和独立临时目录，不读取真实稿件、不向检索服务提交材料、不默认启用全局 Hook。模型推理仍会使用当前 Grok 服务，因此“离线夹具”仅表示不检索外部来源，不表示模型离线运行。

轨迹必须包含真实 init 工具清单、工具调用和结果、正常终止以及相应产物；单纯回答预设标记不得证明权限或执行成功。超时、拒绝、工具不可用和材料不足各自记录，不提高权限强行通过。发布以实际完成范围为准，未通过前不勾选。

以下保留 v0.3.0 架构计划作为历史背景。

### 本轮交付边界

修复及证据工具保存在 `codex/v0.3.1-runtime-evidence` 候选分支。真实运行使用已安装的v0.3.0作为兼容性基线，候选v0.3.1仅完成本地测试与隔离安装；不能混用这两类证据。严格工具隔离或真实交接未通过时，不覆盖main、不更新全局安装、不创建新稳定Release。后续优先复验Grok子Agent实际工具表与交接产物，再做Hook宿主集成，最后重跑四条路由。
